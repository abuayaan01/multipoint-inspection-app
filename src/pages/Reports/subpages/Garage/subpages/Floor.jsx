...
function Floor({ toggleRefresh, garageData, comments }) {
  const [data, setData] = useState(garageData.floor);
  ...
  const onSubmit = async (data) => {
    setLoading(true);
    let reqData = {
      floor: { ...data, extra: extra },
    };
    await updatePropertyReq(id, garageId, reqData, "garage", "floor", photos)
      .then((res) => {
        if (res.status == 200) {
          Swal.fire({
            icon: "success",
  ...
  const materialOptions = {
    label: "Material",
    name: "material",
    options: ["Concrete", "Gravel", "Asphalt", "Dirt"],
  };
  const conditionOptions = {
    ...
    options: [
      "Satisfactory",
      "Typical cracks",
      "Large setling cracks",
      "Recommend repair/replace",
      "Recommend painting",
    ],
  };
  const sourcenOptions = {
    label: `Source of ignition within 18" of the foor`,
    name: "sourceOfIgnition",
    options: ["N/A", "Yes", "No"],
  };

  const Options = [materialOptions, conditionOptions, sourcenOptions];
