...
function Siding({ toggleRefresh, garageData, comments }) {
  const [data, setData] = useState(garageData.siding);
  ...
  const onSubmit = async (data) => {
    setLoading(true);
    let reqData = {
      siding: { ...data, extra: extra },
    };
    await updatePropertyReq(id, garageId, reqData, "garage", "siding", photos)
      .then((res) => {
        if (res.status == 200) {
          Swal.fire({
            icon: "success",
  ...
  return (
    <>
      <form onSubmit={handleSubmit(onSubmit)} className="editForm">
        <p className="headTitle">Siding</p>
