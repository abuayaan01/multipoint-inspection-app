...
function GuttersEavestrough({ toggleRefresh, garageData, comments }) {
  const [data, setData] = useState(garageData.guttersEavestrough);
  ...
  const onSubmit = async (data) => {
    setLoading(true);
    let reqData = {
      guttersEavestrough: { ...data, extra: extra },
    };
  ...
  const conditionOptions = {
    label: "Condition",
    name: "condition",
    options: ["Satisfactory", "Marginal", "Poor", "Same as house"],
  };
  ...
  return (
    <>
      <form onSubmit={handleSubmit(onSubmit)} className="editForm">
        <p className="headTitle">Gutters Eavestrough</p>
        ...
        <Controller
          ...
          render={({ field: { value, onChange } }) => (
            <Editor
              ...
              value={value}
              onChange={onChange}
              comment={comments?.guttersEavestrough}
            />
          )}
        />
...
export default GuttersEavestrough;
