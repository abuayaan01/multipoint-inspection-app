...
function ExteriorServiceDoor({ toggleRefresh, garageData, comments }) {
  const [data, setData] = useState(garageData.exteriorServiceDoor);
  ...
  const onSubmit = async (data) => {
    setLoading(true);
    let reqData = {
      exteriorServiceDoor: { ...data, extra: extra },
    };
  ...
  const conditionOptions = {
    label: "Condition",
    name: "condition",
    options: ["Satisfactory", "Marginal", "Poor", "Damaged/Rusted"],
  };

  const Options = [availabilityOptions, conditionOptions];

  return (
    <>
      <form onSubmit={handleSubmit(onSubmit)} className="editForm">
        <p className="headTitle">Exterior Service Door</p>
        ...
        <Controller
          ...
          render={({ field: { value, onChange } }) => (
            <Editor
              ...
              value={value}
              onChange={onChange}
              comment={comments?.exteriorServiceDoor}
            />
          )}
        />
...
export default ExteriorServiceDoor;
