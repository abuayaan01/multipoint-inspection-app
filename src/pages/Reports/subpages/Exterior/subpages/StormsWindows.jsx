...
function StormsWindows({ toggleRefresh, exterior ,comments}) {
  const [data, setData] = useState(exterior.stormsWindows);
  ...
  const onSubmit = async (data) => {
    setLoading(true);
    let reqData = {
      stormsWindows: { ...data, extra: extra },
    };
    await updatePropertyReq(id, exteriorId, reqData,'exterior', "stormsWindows", photos)
      .then((res) => {
        if (res.status == 200) {
          Swal.fire({
            icon: "success",
  ...
  const materialOptions = {
    label: "Material",
    name: "material",
    options: ["Wood", "SlClad comb.ate", "Wood/Metal comb.", "Metal"],
  };
  const puttyOptions = {
    label: "Putty",
    name: "putty",
    options: ["Satisfactory", "Needed", "N/A"],
  };
  const Options = [availabilityOptions, materialOptions, conditionOptions];
  return (
    <>
      <form onSubmit={handleSubmit(onSubmit)} className="editForm">
        <p className="headTitle">Storms Windows</p>
        ...
        <Controller
          ...
          render={({ field: { value, onChange } }) => (
            <Editor label={"Comments"} value={value} onChange={onChange} comment={comments?.stormWindows}/>
          )}
        />
