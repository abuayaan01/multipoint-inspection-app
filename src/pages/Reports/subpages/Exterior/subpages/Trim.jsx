...
function Trim({ toggleRefresh, exterior,comments }) {
  const [data, setData] = useState(exterior.trim);
  ...
  const onSubmit = async (data) => {
    ...
    await updatePropertyReq(id, exteriorId, reqData,'exterior', "trim", photos)
      .then((res) => {
        if (res.status == 200) {
          Swal.fire({
            icon: "success",
      ...
      <form  onSubmit={handleSubmit(onSubmit)} className="editForm">
        ...
        <Controller
          ...
          render={({ field: { value, onChange } }) => (
            <Editor label={"Comments"} value={value} onChange={onChange} comment={comments?.trim}/>
          )}
        />
