...
function Fascia({ toggleRefresh, exterior ,comments}) {
  const [data, setData] = useState(exterior.fascia);
  ...
  const onSubmit = async (data) => {
    setLoading(true);
    let reqData = {
      fascia: { ...data, extra: extra },
    };
    await updatePropertyReq(id, exteriorId, reqData,'exterior', "fascia", photos)
      .then((res) => {
        if (res.status == 200) {
          Swal.fire({
            icon: "success",
  ...
  return (
    <>
      <form onSubmit={handleSubmit(onSubmit)} className="editForm">
        <p className="headTitle">Fascia</p>
        ...
        <Controller
          ...
          render={({ field: { value, onChange } }) => (
            <Editor label={"Comments"} value={value} onChange={onChange} comment={comments?.fascia}/>
          )}
        />
