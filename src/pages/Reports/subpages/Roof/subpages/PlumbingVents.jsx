...
function PlumbingVents({ toggleRefresh, roof ,comments}) {
  const [data, setData] = useState(roof.plumbingVents);
  ...
  const onSubmit = async (data) => {
    setLoading(true);
    let reqData = {
      plumbingVents: { ...data, extra: extra },
    };
    await updatePropertyReq(inspectionId, roofId, reqData,'roof', "plumbingVents", photos)
      .then((res) => {
        if (res.status == 200) {
          Swal.fire({
            icon: "success",
  ...
  const plumbingVentsCheckboxes = [visibiltyOptions, conditionOptions];
  ...
  return (
    <>
      <form onSubmit={handleSubmit(onSubmit)} className="editForm">
        ...
        <div className="mt-5">
          {plumbingVentsCheckboxes.map((CheckElem, idx) => {
            return (
        ...
        <Controller
          ...
          render={({ field: { value, onChange } }) => (
            <Editor label={"Comments"} value={value} onChange={onChange} comment={comments?.plumbingVents}/>
          )}
        />
