...
function General({ toggleRefresh, roof,comments }) {
  const [data, setData] = useState(roof.general);
  ...
  const onSubmit = async (data) => {
    setLoading(true);
    let reqData = {
      general: { ...data, extra: extra },
    };
    await updatePropertyReq(inspectionId, roofId, reqData,'roof', "general", photos)
      .then((res) => {
        if (res.status == 200) {
          Swal.fire({
            icon: "success",
  ...
  const inspectedFromOptions = {
    label: "Inspected From",
    name: "inspectedFrom",
    options: ["Roof", "Ladder at eaves", "Ground", "With Binoculars"],
  };
  const generalCheckboxes = [visibiltyOptions, inspectedFromOptions];
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="editForm">
        ...
        <Controller
          ...
          render={({ field: { value, onChange } }) => (
            <Editor label={"Comments"} value={value} onChange={onChange} comment={comments?.general}/>
          )}
        />
