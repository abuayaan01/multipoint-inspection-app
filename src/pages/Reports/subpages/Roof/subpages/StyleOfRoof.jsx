...
function StyleOfRoof({ toggleRefresh, roof ,comments}) {
  const [data, setData] = useState(roof.styleOfRoof);
  ...
  const onSubmit = async (data) => {
    setLoading(true);
    let reqData = {
      styleOfRoof: { ...data, extra: extra },
    };
    await updatePropertyReq(inspectionId, roofId, reqData,'roof', "styleOfRoof", photos)
      .then((res) => {
        if (res.status == 200) {
          Swal.fire({
            icon: "success",
  ...
  const typeOptions = {
    label: "Type",
    name: "type",
    options: ["Gable", "Hip", "Mansard", "Shed", "Flat"],
  };
  const pitchOptions = {
    label: "Pitch",
    name: "pitch",
    options: ["Low", "Medium", "Steep", "Flat"],
  };
  const skylightsCheckboxes = [typeOptions, pitchOptions];
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="editForm">
        ...
        <Controller
          ...
          render={({ field: { value, onChange } }) => (
            <Editor label={"Comments"} value={value} onChange={onChange} comment={comments?.styleOfRoof}/>
          )}
        />
