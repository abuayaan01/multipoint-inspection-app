...
function Access({ toggleRefresh, crawlSpaceData, comments }) {
  const [data, setData] = useState(crawlSpaceData.access);
  ...
  const locationOptions = {
    label: "Location",
    name: "location",
    options: ["Exterior", "Interior hatch/door", "Via basement", "No access"],
  };

  const inspectedFromOptions = {
    label: "Inspected From",
    name: "inspectedFrom",
    options: ["Access panel", "In the crawl space"],
  };

  const Options = [locationOptions, inspectedFromOptions];

  return (
    <>
      <form onSubmit={handleSubmit(onSubmit)} className="editForm">
        <p className="headTitle">Access</p>
