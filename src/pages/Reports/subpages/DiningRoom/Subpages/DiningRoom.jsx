...
function DiningRoom({ toggleRefresh, diningRoomData, comments }) {
  const [data, setData] = useState(diningRoomData.diningRoom);
  const [loading, setLoading] = useState(false);
  const [extra, setExtra] = useState([]);
  const [photos, setPhotos] = useState([]);
  const { id, diningRoomId } = useParams();
  const { register, handleSubmit, setValue, control } = useForm();

  const onSubmit = async (data) => {
    setLoading(true);
    let reqData = {
      diningRoom: { ...data, extra: extra },
    };
  ...
  return (
    <>
      <form onSubmit={handleSubmit(onSubmit)} className="editForm">
        <p className="headTitle">Dining Room</p>
