...
function EvaporatorCoilSectionUnit1({
  toggleRefresh,
  electricCoolingSystemData,
  comments,
}) {
  const [data, setData] = useState(
    electricCoolingSystemData.evaporatorCoilSectionUnit1
  );
  ...
  const onSubmit = async (data) => {
    setLoading(true);
    let reqData = {
      evaporatorCoilSectionUnit1: { ...data, extra: extra },
    };
  ...
  return (
    <>
      <form onSubmit={handleSubmit(onSubmit)} className="editForm">
        <p className="headTitle">Evaporator Coil Section Unit #1</p>
        ...
        <Controller
          ...
          render={({ field: { value, onChange } }) => (
            <Editor
              ...
              value={value}
              onChange={onChange}
              comment={comments?.evaporatorCoilSectionUnit1}
            />
          )}
        />
...
export default EvaporatorCoilSectionUnit1;
