...
function StairsStepsBalconies({ toggleRefresh, interiorData, comments }) {
  const [data, setData] = useState(interiorData.stairsStepsBalconies);
  ...
  const onSubmit = async (data) => {
    setLoading(true);
    let reqData = {
      stairsStepsBalconies: { ...data, extra: extra },
    };
  ...
  const Options = [
    ...
    },
    {
      label: "Conditiion",
      name: "condition",
      options: ["Satisfactory", "Marginal", "Poor", "Loose/Missing"],
    },
    {
      label: "Handrail",
      name: "handrail",
      options: [
        ...
        "Poor",
        "Safety hazard",
        "Hand Rail/Railing/Balusters recommended",
      ],
    },
    {
      label: "Risers/Treads",
  ...
  return (
    <>
      <form onSubmit={handleSubmit(onSubmit)} className="editForm">
        <p className="headTitle">Stairs/Steps/Balconies</p>
        ...
        <Controller
          ...
          render={({ field: { value, onChange } }) => (
            <Editor
              ...
              value={value}
              onChange={onChange}
              comment={comments?.stairsStepsBalconies}
            />
          )}
        />
...
export default StairsStepsBalconies;
