...
function Floor({ toggleRefresh, kitchenData, comments }) {
  const [data, setData] = useState(kitchenData.floor);
  ...
  const onSubmit = async (data) => {
    ...
    await updatePropertyReq(id, kitchenId, reqData, "kitchen", "floor", photos)
      .then((res) => {
        if (res.status == 200) {
          Swal.fire({
            icon: "success",
  ...
  const option1 = {
    label: "Condition",
    name: "condition",
    options: ["Satisfactory", "Marginal", "Poor", "Sloping", "Squeaks"],
  };
