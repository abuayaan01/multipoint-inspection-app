...
function LivingRoom({ toggleRefresh, livingRoomData, comments }) {
  const [data, setData] = useState(livingRoomData.livingRoom);
  const [loading, setLoading] = useState(false);
  const [extra, setExtra] = useState([]);
  const [photos, setPhotos] = useState([]);
  const { id, livingRoomId } = useParams();
  const { register, handleSubmit, setValue, control } = useForm();

  const onSubmit = async (data) => {
    setLoading(true);
    let reqData = {
      livingRoom: { ...data, extra: extra },
    };
  ...
  return (
    <>
      <form onSubmit={handleSubmit(onSubmit)} className="editForm">
        <p className="headTitle">Living Room</p>
