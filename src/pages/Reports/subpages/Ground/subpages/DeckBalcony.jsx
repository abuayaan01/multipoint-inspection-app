...
function DeckBalcony({ toggleRefresh, groundData ,comments}) {
  const [data, setData] = useState(groundData.deckBalcony);
  ...
  const onSubmit = async (data) => {
    setLoading(true);
    const reqData = { deckBalcony: { ...data, extra: extra } };
  ...
  const conditionOptions = {
    label: "Condition",
    name: "condition",
    options: ["Satisfactory", "Marginal", "Poor", "Wood in contact with soil"],
  };
  const finishOptions = {
    label: "Finish",
    name: "finish",
  ...
  return (
    <>
      <form onSubmit={handleSubmit(onSubmit)} className="editForm">
        <p className="headTitle">Deck/Balcony</p>
        ...
        <Controller
          ...
          render={({ field: { value, onChange } }) => (
            <Editor label={"Comments"} value={value} onChange={onChange} comment={comments?.deckBalcony}/>
          )}
        />
