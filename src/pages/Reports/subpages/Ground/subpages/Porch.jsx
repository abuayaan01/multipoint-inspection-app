...
function Porch({ toggleRefresh, groundData,comments }) {
  const [data, setData] = useState(groundData.porch);
  ...
  const onSubmit = async (data) => {
    setLoading(true);
    let reqData = { porch: {...data, extra : extra} };
    await updatePropertyReq(inspectionId, groundId, reqData,'grounds', "porch", photos)
      .then((res) => {
        if (res.status == 200) {
          Swal.fire({
            icon: "success",
  ...
  const supportPierOptions = {
    label: "Support Pier",
    name: "supportPier",
    options: ["Concrete", "Wood"],
  };
  ...
  const floorOptions = {
    label: "Floor",
    name: "floor",
    options: ["Satisfactory", "Marginal", "Poor", "Safety Hazard"],
  };
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="editForm">
        ...
        <Controller
          ...
          render={({ field: { value, onChange } }) => (
            <Editor label={"Comments"} value={value} onChange={onChange} comment={comments?.porch}/>
          )}
        />
