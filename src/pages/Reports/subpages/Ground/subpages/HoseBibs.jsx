...
function HoseBibs({ toggleRefresh, groundData,comments }) {
  const [data, setData] = useState(groundData.hoseBibs);
  ...
  const onSubmit = async (data) => {
    setLoading(true);
    const reqData = { hoseBibs: { ...data, extra: extra } };
    await updatePropertyReq(inspectionId, groundId, reqData,'grounds', "hoseBibs", photos)
      .then((res) => {
        if (res.status == 200) {
          Swal.fire({
            icon: "success",
  ...
  const operableOptions = {
    label: "Operable",
    name: "operable",
    options: ["Yes", "No", "Not Tested", "Not On"],
  };
  ...
  return (
    <>
      <form onSubmit={handleSubmit(onSubmit)} className="editForm">
        <p className="headTitle">Hose Bibs</p>
        ...
        <Controller
          ...
          render={({ field: { value, onChange } }) => (
            <Editor label={"Comments"} value={value} onChange={onChange} comment={comments?.hoseBibs}/>
          )}
        />
